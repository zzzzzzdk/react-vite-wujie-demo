...
export default function (props: StayListProps) {
  ...
  const mark = (arr: ResultRowType[], index: number) => {
    if (arr.length === 2) {
      if (arr[0].lngLat.lat === arr[1].lngLat.lat && arr[0].lngLat.lng === arr[1].lngLat.lng) {
        if(index==1){
  ...
  const handleImgClick = (event: React.MouseEvent, item: ResultRowType, index: number) => {
  ...
  return (
    <div className={classnames("card-detail-itemt", {
      active: active
    })}
      onClick={onCardClick}>
      {
      ...
      <div className="infos-con">
        {
          cardData.vehicleInfo.map((item: ResultRowType, index: number, arr: ResultRowType[]) => {
            return (
              <div
      ...
      <div className='sss'>
        {
          cardData.vehicleInfo.length !== 2 ?
            <div className="item-location">
              <Icon type="shijian" />
              <div className="item-location-content">{cardData.vehicleInfo[0].captureTime}</div>
            </div>
            :
            <div className="item-location">
              <Icon type="shijian" />
              <div className="item-location-content">{cardData.vehicleInfo[0].captureTime} - {cardData.vehicleInfo[1].captureTime}</div>
            </div>
        }
        <div className="item-location">
          <Icon type="shichangicon" />
          <div className="item-location-content">落脚时长 { cardData.duration || '-'}</div>
        </div>
      </div>
    </div>)
}
