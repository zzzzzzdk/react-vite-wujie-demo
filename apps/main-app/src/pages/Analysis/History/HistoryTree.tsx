...
import { HistoryTreeProps, HistoryTreeItem, appendStatusType } from "./interface";
...
const HistoryTree = (props: HistoryTreeProps) => {
  const {
    prefixCls,
    onSelect,
  } = props
  const {
    onHistoryTreeDataChange
  ...
  const [treeData, setTreeData] = useState<HistoryTreeItem[]>([])
  // 缓存treeData
  ...
  const [currentData, setCurrentData] = useState<HistoryTreeItem | null>(null)
  ...
  // // 递归格式化数据
  const formatData = (data: HistoryTreeItem[]) => {
    let result = [...data]
    function dg(arr: HistoryTreeItem[], j?: string, level?: number, permission?: number) {
      level = level || 0
      level++
      arr.forEach((item, i) => {
  ...
  const getData = () => {
    setAjaxLoading(true)
    services.history.getJobList<any, any>().then(res => {
      // console.log(res)
      setAjaxLoading(false)
      const resData = formatData(res.data || [])
      onHistoryTreeDataChange?.(resData)
      setTreeData(resData)
    }).catch(err => {
      console.log(err)
    })
  ...
  // 是否有重复数据
  const isTestCommonName = (data: HistoryTreeItem[], text = 'name') => {
  ...
  // 添加任务
  const addTaskCommonFun = (indexArray: string[], type = 'one', item?: HistoryTreeItem) => {
  ...
  // 点击编辑某一项名称
  const handleEditItem = (item: HistoryTreeItem, e: React.MouseEvent) => {
    e.stopPropagation()
    if (appendStatus.current.status) {
      Message.warning('请完成本次操作')
      return
  ...
  // 取消新增/编辑状态
  const handleCancelEdit = (item: HistoryTreeItem, e: React.MouseEvent) => {
    e.stopPropagation()
    let arr = [...treeData]
    if (appendStatus.current.isFirst) {
      arr.splice(0, 1)
      onHistoryTreeDataChange?.(arr)
      setTreeData(arr)
    } else {
      // console.log(cacheTreeData.current)
      onHistoryTreeDataChange?.(cacheTreeData.current)
  ...
  // 编辑确认保存
  const handleConfirmEdit = (item: HistoryTreeItem, e: React.MouseEvent) => {
    e.stopPropagation()
    ...
    if (appendStatus.current.type === 'add') {
      ...
      services.history.updateJob({
        ...
        const newTreeData = formatData(arr)
        onHistoryTreeDataChange?.(newTreeData)
        setTreeData(newTreeData)
        cacheTreeData.current = newTreeData
  ...
  // 点击添加子项
  const handleAddItem = (item: HistoryTreeItem) => {
    if (appendStatus.current.status) {
      Message.warning('请完成本次操作')
      return
    }
  ...
  // 点击删除某一项
  const handleDelItem = (item: HistoryTreeItem) => {
    ...
    services.history.delJob({ jobId: item.jobId }).then(res => {
  ...
  const handleNestedItem = (treeShapeArray: HistoryTreeItem[], indexArray: string[], newChildren: HistoryTreeItem, type = 'add'): HistoryTreeItem | undefined => {
    // 检查是否已经遍历到了最后一层
    if (indexArray.length === 0) {
      return undefined;
    }
  ...
  // 处理数据权限
  const handleItemAuth = (item: HistoryTreeItem) => {
    let indexArray = item.__index?.split('-') || []
    appendStatus.current._index = indexArray
    setCurrentData(item)
  ...
  const handleSharePermissionOk = (data: { privilege: number, sharedUsers: Receiver[] }) => {
    console.log(data)
    updateTip('loading', '正在保存...', 0)
    const oldData = currentData as HistoryTreeItem
  ...
  const renderItemTitle = (item: HistoryTreeItem) => {
  ...
  const handleRenderTree = (data: HistoryTreeItem[]) => {
    if (data && data.length) {
