...
import { ListMoreBtn } from "@/components";
...
const TargetResult = (props: TargetResultProps) => {
    ...
    },
    {
      ...
      render: (text, record, index) => (
        record.licensePlate1Url ?
          <Link href={record.licensePlate1Url}>{parseStr(text)}</Link>
          : <span>{parseStr(text)}</span>
      )
    },
    {
      title: '二次识别车牌',
    ...
    return (
      <GroupTable
        ...
        tableConfig={{
          ...
          countTitle: ajaxFormData?.targetType === 'vehicle' ? '过车数量' : '数量'
        }}
  ...
  return (
    <div className={classNames("result-group", {
      // "first": firstLoading
    })}>
      <ResultBox
        loading={loading}
