...
// import Graph from '@/config/relation'
...
export default () => {
  ...
  const changeNodeStatus = (type: string) => {
    graph.current.changeStatus(type)

  }
  useEffect(() => {
    // graph.current = new Graph('relationBox', {
    //   message: () => { },
    //   changeCurrent: () => { },
    //   changeLayout: () => { }
    // })
  ...
  const handleLayout = (type: string) => {
    graph.current.changeLayout(type)
  ...
  return <div style={{ height: '100%' }}>
    <div className="tool-box">
      ...
      <div className="tool-box-layout">
        <div onClick={() => handleLayout('grid')}>
          <p>矩形</p>
        </div>
        <div onClick={() => handleLayout('circle')}>
          <p>圆形</p>
        </div>
        {/* <div className={layoutType === 'tree' ? "tool-item tool-item-active" : "tool-item"} onClick={() => handleLayout('tree')}>
                  <Icon type="-cengci" />
                  <p>层次</p>
                </div> */}
        <div onClick={() => handleLayout('default')}>
          <p>网格</p>
        </div>
      </div>
    </div>
    <div className="legend-box" style={{}}>
      <div className="follow-legend">
        <div className="follow-legend-box" onClick={() => changeNodeStatus('2')}>

        </div>
        <div>伴随数据</div>
      </div>
      <div className="belongs-legend">
        <div className="belongs-legend-box" onClick={() => changeNodeStatus('1')}>

        </div>
        <div>登记数据</div>
      </div>
    </div>
    <div className="relation-chart" id="relationBox" style={{}}></div>
    {/* <div>
      <Button onClick={() => {
        graph.updateLayout({
          type: 'circular',
