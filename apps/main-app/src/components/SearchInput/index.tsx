...
import { Icon, CloseCircleFilled, RightOutlined } from '@yisa/webui/es/Icon'
...
import importDataPng from '@/assets/images/record/icon-import-data.png'
import approvePng from '@/assets/images/record/icon-approve.png'
import blacklistPng from '@/assets/images/record/icon-black-list.png'
...
const SearchInput = forwardRef((props: any, ref) => {
  ...
  const prefixCls = 'record-search-input'
  ...
  const [blackListAuth, setBlackListAuth] = useState(false)
  ...
  useEffect(() => {
    ajax.record.getAdminAuth<null, boolean>().then(res => {
      setBlackListAuth(!!res.data)
    }).catch(() => {
      setBlackListAuth(false)
    })
  }, [])
  ...
  return <div className={`${prefixCls} ${className}`}>
    ...
    <div className={`search-input ${isMore == 'open' ? 'search-input-disabled' : ''}  ${searchType == 'search' ? '' : 'search-input-big'}`}>
            ...
            searchForm.activeMoreKey == 'person' ?
              <div className="search-items">
                <Form layout="vertical">
                  ...
                  <Form.Item className="native-place" colon={false} label={'户籍地址'}>
                    <div className="place-box">
                      <Input
                        disabled={formData.noLocal}
                  ...
                  <Form.Item className="detail-address" colon={false} label={'现住址'}>
                    <Input
                  ...
                  >
                    <div className="times-box">
                      <Select
                        ...
                        getTriggerContainer={() => document.querySelector('.times-box') as HTMLElement}
                      />
                      <FormInputGroup
      ...
      </div>
      {
        searchForm.searchType
          ...
          : <Input type="text"
            ...
            placeholder="多关键词检索姓名/证件号/性别/车牌号码/户籍地址/现住址"
      ...
      <div className={`search-btn`} onClick={handleSearch}>
        {
          searchType == 'search' ? <Icon type="dangansousuo" /> : '查询'
        }
      </div>
    </div>
        ...
        <div className="history-info">
          <div>历史:</div>
          {
            historySearch.map((ele: any, index: number) => {
              return <div key={index} onClick={() => handleClickHistoryItem(ele)}
                title={ele}
    ...
    {
      searchType == 'search' && <div className="more-btn">
        ...
        <div className="more-btn-item" onClick={() => { onImportData?.("person") }}>
          <img src={importDataPng} alt="" />
          <div>
            <span className="title">导入数据</span>
            <span className="sub-title">开始导入<RightOutlined /></span>
          </div>
        </div>
        <div className="more-btn-item" onClick={onAuthApprove}>
          <img src={approvePng} alt="" />
          <div>
            <span className="title">权限审批</span>
            <span className="sub-title">进入审批<RightOutlined /></span>
          </div>
        </div>
        {blackListAuth && <BlackList icon={blacklistPng} />}
      </div>
    }
  </div>
})
