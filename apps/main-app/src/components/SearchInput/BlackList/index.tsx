...
import { Button, Tabs, Tag, Modal, Pagination, Checkbox, Loading, Tooltip } from "@yisa/webui"
...
const BlackList = (props: any) => {
  ...
  // 获取抓拍数据数据
  const getPortraitFaceData = (data: {
    id: string;
    pageNo: number;
    pageSize: number;
  }) => {
    ...
    ajax.record.getPortraitFaceData<any, any[]>({
      ...
      pageSize: data.pageSize || character.pageSizeOptions[0],
      ...(blackActiveKey === '1' ? {
        groupId: data.id,
      } : {}),
      ...(blackActiveKey === '2' ? {
        groupPlateId: data.id
      } : {})
    })
      .then(res => {
        setBlackAjaxLoading(false)
    ...
    >
      <div className="black-body">
        ...
        <div className="black-title">
          ...
          {
            captureCount ? <Tag closable onClose={() => {
              setBlackPage(defaultPage)
              getBlackListsData(defaultPage)
              setGroupId('')
            }}>抓拍图像：{captureCount}</Tag>
              : ''
          }
        </div>
