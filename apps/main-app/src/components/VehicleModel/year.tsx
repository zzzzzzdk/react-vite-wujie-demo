...
function Year(props: brandListProps) {
  const {
    prefixCls
  } = props
  const {
  ...
  const placeholder = isArray(searchPlaceholder) ? searchPlaceholder[2] || '搜索' : searchPlaceholder
  ...
  const data = useMemo(() => {
    if (!yearData) return []
    const _modelData = isArray(modelValue) ? (modelValue[0] || undefined) : undefined
    const _data = yearData[_modelData as value] || []
    return _data.filter(elem => `${elem?.v}`.toLowerCase().includes(filterText.toLowerCase()))
  }, [modelValue, yearData, filterText])
  ...
  const handleChangeBrand = (value: value) => {
    ...
    if (isActive) {
      onChange && onChange(brandValue, modelValue || [], [...values, value])
    } else {
      onChange && onChange(brandValue, modelValue || [], values.filter(elem => elem !== value))
    }
  }
